...
async function generateWithRetry(params: Parameters<typeof ai.models.generateContent>[0], retries = 3): ReturnType<typeof ai.models.generateContent> {
  for (let i = 0; i < retries; i++) {
    ...
    catch (e: unknown) { const status = (e as { status?: number }).status; if (status === 429 && i < retries - 1) { console.log(`  Rate limited, waiting ${15*(i+1)}s...`); await new Promise(r => setTimeout(r, 15000*(i+1))); continue; } throw e; }
  }
...
const scenes = [
  { name: "scene-1-hook", script: "Your newborn is basically blind. And it's completely on purpose.", direction: "warm, intriguing, like revealing a fascinating secret" },
  { name: "scene-2-body", script: "Newborns can only see clearly about eight to twelve inches from their face. That's exactly the distance to your face when you're holding them. By three months, they can track moving objects. And by six months? Their color vision is almost fully developed." },
  { name: "scene-3-cta", script: "Follow for more baby facts that'll blow your mind." },
];
